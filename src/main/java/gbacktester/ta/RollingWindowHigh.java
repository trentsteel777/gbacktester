...
import gbacktester.service.CsvWriterService.StockRecord;

/**
 * Maintains a rolling-window max of the "high" field in StockRecord
...
public class RollingWindowHigh {
     ...
     *
     * @param prices The full list of StockRecord
     * @param i      The current index in that list
     * @return       true if `prices.get(i).high` is the max in [i - (windowSize-1) .. i]
     */
    public boolean updateAndCheckIsHigh(List<StockRecord> prices, int i) {
        double currentHigh = prices.get(i).high;

        // 1) Remove from back while this bar's high >= the back's high
        while (!maxDeque.isEmpty() && prices.get(maxDeque.getLast()).high <= currentHigh) {
            maxDeque.removeLast();
        }
